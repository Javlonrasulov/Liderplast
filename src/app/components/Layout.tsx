...
function LanguageDropdown() {
  ...
  return (
    <div className="relative z-[80]" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full z-[90] mt-2 w-[min(13rem,calc(100vw-1rem))] max-w-[calc(100vw-1rem)] rounded-2xl border border-slate-200 bg-white py-1.5 shadow-2xl shadow-slate-200/60 dark:border-slate-700 dark:bg-slate-800 dark:shadow-black/40 overflow-hidden">
...
export function Layout() {
  ...
  return (
    <div className="flex h-screen min-w-0 overflow-hidden bg-slate-50 dark:bg-slate-950">
      <AccountCredentialsDialog open={credentialsOpen} onOpenChange={setCredentialsOpen} />
      {/* Mobile overlay */}
      {mobileOpen && (
      ...
      <div className={`flex min-h-0 min-w-0 flex-1 flex-col transition-all duration-300 ${collapsed ? 'lg:ml-16' : 'lg:ml-64'}`}>
        ...
        <header className="z-10 flex min-h-14 shrink-0 items-center gap-2 overflow-visible border-b border-slate-200 bg-white px-2 py-2 dark:border-slate-700 dark:bg-slate-900 sm:px-3 lg:h-14 lg:px-5 lg:py-0">
          ...
          <div className="flex min-w-0 flex-1 items-center justify-end gap-1 sm:gap-2">
            {lowStock && (
              <div className="hidden min-w-0 items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-2 py-1.5 dark:border-amber-700 dark:bg-amber-900/20 md:flex md:max-w-[min(40%,14rem)] md:shrink">
                <span className="h-1.5 w-1.5 shrink-0 animate-pulse rounded-full bg-amber-500" />
                <span className="truncate text-xs font-medium text-amber-700 dark:text-amber-400">{t.layoutMaterialLow}</span>
              </div>
            )}

            {/* Mobile: compact toolbar + "More" sheet */}
            <div className="flex shrink-0 items-center justify-end gap-1 sm:hidden">
              ...
              <Sheet>
                ...
                <SheetContent side="right" className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 p-0">
                  <SheetHeader className="border-b border-slate-100 dark:border-slate-800">
                    <SheetTitle className="text-slate-900 dark:text-white">{pageTitle}</SheetTitle>
                  </SheetHeader>
                  <div className="p-4 space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t.dfTitle}</span>
                      <DateFilterPicker />
                    </div>

                    <div className="flex items-center justify-between gap-3">
                      <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t.langName}</span>
                      <LanguageDropdown />
                    </div>

                    <div className="flex items-center justify-between gap-3">
                      <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">A+</span>
                      <FontSizeControl />
                    </div>
                    ...
                    <div className="border-t border-slate-100 dark:border-slate-800 pt-3 flex items-center justify-between gap-2">
                      <button
                        ...
                        className="flex items-center gap-2 rounded-lg px-2 py-2 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                      >
